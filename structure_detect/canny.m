...
% high_thresh -- high threshold for strong edge(normalized to 1).
    ...
    grad_x_direct = grad_x;
    grad_x_direct(grad_x>=0) = 1;
    grad_x_direct(grad_x<0) = -1;
    grad_y_direct = grad_y;
    grad_y_direct(grad_y>=0) = 1;
    grad_y_direct(grad_y<0) = -1;
    x = 1:size(gray_filt,2);
    y = 1:size(gray_filt,1);
    [X, Y] = meshgrid(x,y);
    X_pos = X + grad_x_direct;
    X_neg = X - grad_x_direct;
    Y_pos = Y + grad_y_direct;
    Y_neg = Y - grad_y_direct;
    if 1
        X_pos = X;
        X_neg = X;
        Y_pos = Y;
